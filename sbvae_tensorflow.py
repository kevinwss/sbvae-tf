...
def Q(X):
    ...
    a = tf.matmul(h, Q_W2_mu) + Q_b2_mu #(batch,100)
    b = tf.matmul(h, Q_W2_sigma) + Q_b2_sigma   #(batch,100)
...
def sample_sb(a,b,mb_size):
    ...
    u = tf.random_normal(shape=tf.shape(a))
...
def compute_KL(prior_alpha, prior_beta,a,b):
    #kl = 0
    #for i in range():
    ...
    kl += tf.math.log(a*b) + tf.math.log(Beta_fn(prior_alpha, prior_beta))  #做出来和a,b一样大的tf.constant来
...
kl_loss = compute_KL(2,2,a,b)  #stick breaking KL divergence
# VAE loss
...
for it in range(1000000):
    ...
    _, loss,kl_loss_,recon_loss_, a_, b_,z_sample_,logits_ = sess.run([solver, vae_loss,kl_loss,recon_loss,a,b,z_sample,logits], feed_dict={X: X_mb})
